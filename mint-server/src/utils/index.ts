...
import { db } from "../db"
import { track_segments_table, tracks_table } from "../db/schema"
...
// export async function getRaceTrack(raceId: number) {
//   const track = (await db
//     .select()
//     .from(tracks_table)
//     .where(eq(tracks_table.id, raceId)).limit(1))[0]
...
export function isValidUsername(username: string) {
  const regex = /^[A-Za-z][A-Za-z0-9._-]{3,63}$/;
